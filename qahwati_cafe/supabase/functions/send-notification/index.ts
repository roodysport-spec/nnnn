...
serve(async (req) => {
  ...
  try {
    ...
    // 3b. إرسال لكل الـ Web Tokens المحفوظة في Supabase
    const supabaseUrl = Deno.env.get('SUPABASE_URL')
    const supabaseKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')

    if (supabaseUrl && supabaseKey) {
      const sbAdmin = createClient(supabaseUrl, supabaseKey)
      const { data: tokens } = await sbAdmin.from('push_tokens').select('token').eq('platform', 'web')
      
      if (tokens && tokens.length > 0) {
        const tokenResults = await Promise.allSettled(
          tokens.map(({ token }: { token: string }) => sendMessage({ token }))
        )
        results.push({ target: 'web_tokens', count: tokens.length, results: tokenResults })
      }
    }
